...
const selectedCustomerCode = ref('AL RO')
...
const isEnquiryDialogOpen = ref(false)
const enquiryForm = ref<EnquiryFormData>({
  customerCode: '',
  partNumber: '',
  description: '',
  enquiryQuantity: 1,
...
const selectedCustomer = computed(() => {
  return customers.find((customer) => customer.code === selectedCustomerCode.value) ?? customers[0]!
})

const selectedEnquiry = computed(() => {
  return enquiryRecords.find((record) => record.id === selectedEnquiryId.value) ?? null
})
...
function categoryValueFromCode(code: string): string {
  ...
  return categoryMap[code.toUpperCase()] ?? 'ct-parts'
}

function remarkValueFromCode(code: string): string {
  ...
  return remarkMap[code.toUpperCase()] ?? 'de'
}
...
function selectEnquiryRecord(record: EnquiryRecord): void {
  selectedEnquiryId.value = record.id
  partSearchTerm.value = record.partNumber
  selectedPartCategory.value = categoryValueFromCode(record.category)
  selectedPartRemark.value = remarkValueFromCode(record.remarkCode)
  activatePart(record.partNumber)
}

function openNewEnquiryDialog(): void {
  const record = selectedEnquiry.value
  ...
  enquiryForm.value = {
    ...
    enquiryQuantity: record.enquiryQuantity,
  ...
  isEnquiryDialogOpen.value = true
}

function submitDummyEnquiry(): void {
  // Replace this with the real API request when the enquiry endpoint is ready.
  isEnquiryDialogOpen.value = false
}
...
<template>
  <div class="flex h-full w-full min-h-0 min-w-0 max-w-full flex-col overflow-hidden bg-muted/20">
    ...
    >
      <!-- Upper panel: part details, stock, orders, movements and supplier tables -->
      <SplitterPanel
        id="upper-panel"
        :default-size="60"
        :min-size="25"
      ...
      >
        <div class="h-full w-full min-w-0 overflow-x-hidden overflow-y-auto">
          <div class="mx-auto w-full max-w-[1800px] p-3 sm:p-4">
                ...
                >
                  ...
                  >
                    <div class="flex min-w-0 items-center gap-2">
                      <span
                        class="flex size-7 items-center justify-center rounded-md bg-primary/10"
                      >
                ...
                <div class="p-4">
                  <div class="mb-4 flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p class="text-[10px] text-muted-foreground">
                        Recent supplier purchase prices
                      </p>
                    </div>
                  </div>
      ...
      <!-- Lower panel: customer list and enquiry table -->
      <SplitterPanel
        id="lower-panel"
        ref="bottomPanelRef"
        :default-size="40"
      ...
      >
        <div class="flex h-full w-full min-h-0 min-w-0 flex-col overflow-hidden p-3 sm:p-4">
          <div
            class="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"
          >
            <DropdownMenu>
              <DropdownMenuTrigger as-child>
                <Button variant="outline" class="h-10 w-full min-w-0 justify-between sm:w-72">
                  <span class="flex min-w-0 items-center gap-2 overflow-hidden">
                    ...
                    <span class="truncate">{{ selectedMenu.label }}</span>
                    <Badge
          ...
          <div class="mt-3 min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto">
            <div v-if="hasLowerTable" class="grid min-w-0 gap-3 lg:grid-cols-[14rem_minmax(0,1fr)]">
              <!-- Customer table/list -->
              <aside class="min-w-0 overflow-hidden rounded-lg border bg-card shadow-sm">
                ...
                <div class="divide-y">
                  <button
                    ...
                    :key="customer.code"
                    type="button"
                    class="flex w-full min-w-0 items-start gap-2 px-3 py-3 text-left transition-colors hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring"
                    :class="
                      selectedCustomerCode === customer.code ? 'bg-primary/10 text-primary' : ''
                    "
                    @click="selectedCustomerCode = customer.code"
                  >
              ...
              <!-- Main lower records table -->
              <article class="min-w-0 overflow-hidden rounded-lg border bg-card shadow-sm">
                <header
                  class="flex flex-col gap-2 border-b px-4 py-3 sm:flex-row sm:items-center sm:justify-between"
                >
                  <div class="min-w-0">
                    <h3 class="truncate text-sm font-semibold" :title="selectedCustomer.name">
                      {{ selectedCustomer.name }}
                    </h3>
                    <p class="mt-0.5 text-[11px] text-muted-foreground">
                      Select a row to load its part data in the upper panel.
                    </p>
                  </div>
                  <Badge variant="outline">{{ enquiryRecords.length }} records</Badge>
                </header>
                  ...
                  <table class="w-full min-w-[1280px] text-left text-xs">
                    <thead class="sticky top-0 z-[1] bg-muted text-muted-foreground">
                      <tr>
                        ...
                        <th class="px-3 py-2.5 font-medium">Description</th>
                        <th class="px-3 py-2.5 text-right font-medium">Enq. qty</th>
                        <th class="px-3 py-2.5 text-right font-medium">Target price</th>
                        <th class="px-3 py-2.5 font-medium">Target remarks</th>
                        <th class="px-3 py-2.5 font-medium">Warehouse</th>
                        <th class="px-3 py-2.5 text-right font-medium">DE price</th>
                        <th class="px-3 py-2.5 font-medium">Remarks</th>
                        <th class="px-3 py-2.5 text-center font-medium">Web reseller</th>
                    ...
                    <tbody class="divide-y">
                      <tr
                        ...
                        role="button"
                        tabindex="0"
                        :aria-selected="selectedEnquiryId === record.id"
                        class="cursor-pointer transition-colors hover:bg-blue-50/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 dark:hover:bg-blue-950/20"
                        :class="
                          selectedEnquiryId === record.id
                            ? 'bg-blue-50 text-blue-950 shadow-[inset_3px_0_0_0_rgb(59_130_246)] dark:bg-blue-950/30 dark:text-blue-50'
                            : ''
                        "
                        @click="selectEnquiryRecord(record)"
                        @keydown.enter.prevent="selectEnquiryRecord(record)"
                        @keydown.space.prevent="selectEnquiryRecord(record)"
                      >
                        <td class="px-3 py-2.5 font-medium">
                          {{ record.category }}
                        </td>
                        ...
                        <td class="px-3 py-2.5">{{ record.targetRemarks }}</td>
                        ...
                        <td class="px-3 py-2.5 text-center">
                          {{ record.webReseller ? 'Yes' : 'No' }}
                        </td>
                        <td class="whitespace-nowrap px-3 py-2.5">
                          {{ record.enquiryDate }}
                        </td>
                        <td class="px-3 py-2.5">{{ record.assignedTo }}</td>
                        <td class="px-3 py-2.5">
                ...
                <!-- Mobile/tablet record cards: no horizontal page scrollbar -->
                <div class="divide-y lg:hidden">
                  <article
                    ...
                    tabindex="0"
                    :aria-selected="selectedEnquiryId === record.id"
                    class="cursor-pointer p-4 transition-colors hover:bg-blue-50/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 dark:hover:bg-blue-950/20"
                    :class="
                      selectedEnquiryId === record.id
                        ? 'bg-blue-50 shadow-[inset_3px_0_0_0_rgb(59_130_246)] dark:bg-blue-950/30'
                        : ''
                    "
                    @click="selectEnquiryRecord(record)"
                    @keydown.enter.prevent="selectEnquiryRecord(record)"
                      ...
                      <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-1.5">
                          ...
                          <Badge variant="outline"
                            >{{ record.category }} · {{ record.remarkCode }}</Badge
                          >
                        </div>
            ...
            <div
              v-else
              class="flex min-h-48 flex-col items-center justify-center rounded-lg border border-dashed bg-card p-6 text-center"
            >
              <ClipboardListIcon class="size-8 text-muted-foreground" />
              <h3 class="mt-3 text-sm font-semibold">
                No {{ selectedMenu.label.toLowerCase() }} records
              </h3>
              <p class="mt-1 max-w-md text-xs text-muted-foreground">
                Records for this section will appear here when they become available.
              </p>
            </div>
          </div>
          ...
          <footer
            v-if="hasLowerTable"
            class="mt-3 flex min-w-0 shrink-0 flex-col gap-3 border-t pt-3 xl:flex-row xl:items-center xl:justify-between"
          >
            <div
              class="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2 text-[11px] text-muted-foreground"
            >
              <span class="font-medium text-foreground"
                >No. of records: {{ enquiryRecords.length }}</span
              >
              <span class="flex items-center gap-1.5"
                ><i class="size-2.5 rounded-sm bg-red-500" /> Remarks sent</span
              >
              <span class="flex items-center gap-1.5"
                ><i class="size-2.5 rounded-sm bg-emerald-400" /> Target price</span
              >
              <span class="flex items-center gap-1.5"
                ><i class="size-2.5 rounded-sm bg-yellow-400" /> Special price</span
              >
              <span class="flex items-center gap-1.5"
                ><i class="size-2.5 rounded-sm bg-amber-500" /> Mobile app</span
              >
            </div>
            ...
            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-4">
              <Button
                ...
                variant="outline"
                size="sm"
                class="border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 hover:text-blue-800 dark:border-blue-900 dark:bg-blue-950/40 dark:text-blue-300 dark:hover:bg-blue-950/70"
                @click="openNewEnquiryDialog"
              >
    ...
    <Dialog v-model:open="isEnquiryDialogOpen">
      <DialogContent class="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>New enquiry</DialogTitle>
          <DialogDescription>
            Create a dummy enquiry using the selected
            <span class="font-medium text-foreground">
              {{ selectedEnquiry?.partNumber }}
            </span>
            record.
          </DialogDescription>
        </DialogHeader>

        <form class="grid gap-5" @submit.prevent="submitDummyEnquiry">
          <div
            class="rounded-lg border border-blue-100 bg-blue-50/70 p-3 dark:border-blue-900 dark:bg-blue-950/30"
          >
            <p class="text-xs font-medium text-blue-900 dark:text-blue-100">Selected enquiry</p>
            <div
              class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-blue-700 dark:text-blue-300"
            >
              <span>{{ selectedEnquiry?.partNumber }}</span>
              <span>{{ selectedEnquiry?.description }}</span>
              <Badge variant="outline">{{ selectedEnquiry?.status }}</Badge>
            </div>
          </div>

          <div class="grid gap-4 sm:grid-cols-2">
            <div class="grid gap-1.5 sm:col-span-2">
              <label for="new-enquiry-customer" class="text-xs font-medium"> Customer </label>
              <select
                id="new-enquiry-customer"
                v-model="enquiryForm.customerCode"
                class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
            ...
            <div class="grid gap-1.5">
              <label for="new-enquiry-part-number" class="text-xs font-medium"> Part number </label>
              <input
                id="new-enquiry-part-number"
                v-model="enquiryForm.partNumber"
                type="text"
                required
                class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
              />
            </div>

            <div class="grid gap-1.5">
              <label for="new-enquiry-description" class="text-xs font-medium"> Description </label>
              <input
                id="new-enquiry-description"
            ...
            <div class="grid gap-1.5">
              <label for="new-enquiry-quantity" class="text-xs font-medium">
                Enquiry quantity
              </label>
              <input
                id="new-enquiry-quantity"
                v-model.number="enquiryForm.enquiryQuantity"
                type="number"
                min="1"
                required
                class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
            ...
            <div class="grid gap-1.5">
              <label for="new-enquiry-target-price" class="text-xs font-medium">
                Target price (AED)
              </label>
              <input
                id="new-enquiry-target-price"
                v-model.number="enquiryForm.targetPrice"
                type="number"
                min="0"
                step="0.01"
                class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
            ...
            <div class="grid gap-1.5">
              <label for="new-enquiry-warehouse" class="text-xs font-medium"> Warehouse </label>
              <select
                id="new-enquiry-warehouse"
                v-model="enquiryForm.warehouse"
                class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
            ...
            <div class="grid gap-1.5">
              <label for="new-enquiry-assigned-to" class="text-xs font-medium"> Assigned to </label>
              <select
                id="new-enquiry-assigned-to"
                v-model="enquiryForm.assignedTo"
                class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                <option value="">Unassigned</option>
                <option value="M. Khan">M. Khan</option>
                <option value="S. Ali">S. Ali</option>
                <option value="J. Santos">J. Santos</option>
              </select>
            </div>

            <div class="grid gap-1.5">
              <label for="new-enquiry-status" class="text-xs font-medium"> Status </label>
              <select
                id="new-enquiry-status"
                v-model="enquiryForm.status"
                class="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
            ...
            <div class="grid gap-1.5 sm:col-span-2">
              <label for="new-enquiry-remarks" class="text-xs font-medium"> Remarks </label>
              <textarea
                id="new-enquiry-remarks"
                v-model="enquiryForm.remarks"
                rows="3"
                placeholder="Add availability, delivery, or customer notes..."
                class="flex min-h-20 w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
              />
            </div>
          </div>

          <DialogFooter class="gap-2 sm:gap-0">
            <Button type="button" variant="outline" @click="isEnquiryDialogOpen = false">
              Cancel
            </Button>
            <Button type="submit"> Save dummy enquiry </Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
