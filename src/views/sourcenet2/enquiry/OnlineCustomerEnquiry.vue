...
const selectedEnquiryId = ref<number | null>(enquiryRecords[0]?.id ?? null)
const selectedPartNumber = ref(enquiryRecords[0]?.partNumber ?? '')
const selectedCategory = ref(toWorkspaceCategory(enquiryRecords[0]?.category ?? 'CT'))
const selectedPartRemark = ref(toWorkspaceRemark(enquiryRecords[0]?.remarkCode ?? 'DE'))
    ...
    <SplitterGroup direction="vertical" class="h-full min-h-0">
      ...
      <SplitterPanel :default-size="50" :min-size="30" class="min-h-0 overflow-hidden">
        <div class="flex h-full min-h-0 flex-col gap-4 overflow-auto p-4">
          <div>
            <h1 class="text-2xl font-bold">Online Customer Enquiry</h1>
          </div>
